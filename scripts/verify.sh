...
NLB=$(kubectl get svc -n c2-gateway c2-gateway-svc -o jsonpath='{.status.loadBalancer.ingress[0].hostname}')
...
kubectl get pods -n c2-gateway -o wide; kubectl get pods -n c2-app -o wide
RUNNING=$( { kubectl get pods -n c2-gateway --no-headers; kubectl get pods -n c2-app --no-headers; } | awk '$3=="Running"' | wc -l)
...
step 7b "로드밸런싱 — 클러스터 내부(gateway ns)에서 c2-order-svc 직접 호출 20회"
# NetworkPolicy가 gateway 네임스페이스만 허용하므로 검증 파드도 gateway에서 띄운다 (app ns 내부 파드끼리도 차단됨)
INNER=$(kubectl run lb-check --rm -i --restart=Never -n c2-gateway --image=curlimages/curl:8.10.1 -q -- \
  sh -c 'for i in $(seq 20); do curl -s -m 5 http://c2-order-svc.c2-app.svc.cluster.local:8080/api/orders; echo; done' 2>/dev/null | grep -o '"pod":"[^"]*"' | sort | uniq -c)
echo "$INNER" | sed 's/^/  /'
DISTINCT=$(echo "$INNER" | wc -l)
[[ "$DISTINCT" -ge 2 ]] && ok "파드 $DISTINCT종으로 분산" || fail "파드 $DISTINCT종 (기대 2)"

step 8 "NetworkPolicy 우회 차단 — default 네임스페이스에서 c2-order-svc 접근 → 타임아웃"
kubectl run np-check --rm -i --restart=Never -n default --image=curlimages/curl:8.10.1 -q -- \
  curl -s -m 5 -o /dev/null -w '%{http_code}' http://c2-order-svc.c2-app.svc.cluster.local:8080/api/orders > /tmp/np_out 2>&1
NPCODE=$(grep -oE '^[0-9]{3}' /tmp/np_out | head -1)
if [[ "$NPCODE" == "000" || -z "$NPCODE" ]]; then ok "연결 차단됨 (timeout)"; else fail "HTTP $NPCODE — 차단되지 않음"; fi

step 9 "자가 복구 — c2-order-svc 파드 1개 삭제 후 무중단 확인"
VICTIM=$(kubectl get pods -n c2-app -l app=c2-order-svc -o jsonpath='{.items[0].metadata.name}')
kubectl delete pod -n c2-app "$VICTIM" --wait=false >/dev/null
ERRORS=0
for i in $(seq 10); do
  C=$(curl -s -o /dev/null -m 5 -w "%{http_code}" "http://$NLB/api/orders" -H "Authorization: Bearer $TOKEN")
  [[ "$C" != "200" ]] && ERRORS=$((ERRORS+1)); sleep 1
done
kubectl rollout status deploy/c2-order-svc -n c2-app --timeout=120s >/dev/null
kubectl get pods -n c2-app -l app=c2-order-svc
[[ "$ERRORS" -eq 0 ]] && ok "삭제 중 10회 호출 모두 200, 파드 재생성됨" || fail "삭제 중 실패 $ERRORS회"

step 10 "Ingress 비교 — ALB로 토큰 없이 호출 → 200 (JWT 검증 없음)"
ALB=$(kubectl get ingress -n c2-app c2-app-ingress -o jsonpath='{.status.loadBalancer.ingress[0].hostname}' 2>/dev/null)
if [[ -z "$ALB" ]]; then
